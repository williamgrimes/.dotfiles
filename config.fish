...
function poweroff
    set countdown 5
    trap 'echo -e "\nCancelled poweroff"; exit' SIGINT
    echo "Starting poweroff countdown..."
    for i in (seq $countdown -1 1)
        echo -n "Powering off in $i... (Ctrl+C to cancel) \r"
        sleep 1
    end
    echo -e "\nPowering off now!"
    /sbin/poweroff
...
alias bluetooth-connected="bluetoothctl devices Connected"
